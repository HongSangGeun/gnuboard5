...
function wz_get_hangul_date1($date) {
    global $weekstr;

    $parts = explode(' ', trim($date));
    $ymd   = str_replace('-', '', $parts[0]);
    ...
    // 요일 계산
    $w = $weekstr[date('w', strtotime("$y-$m-$d"))];

    // YYYY. MM. DD (요일)
    $date_str = sprintf("%s. %s. %s (%s)", $y, $m, $d, $w);

    // 시간 있으면 붙여서 반환
    return $time ? $date_str . ' ' . $time : $date_str;
}
