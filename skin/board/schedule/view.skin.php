...
<span class="bo_v_cate" style="background-color:<?php echo $cate_color ?>; color:#fff; padding:2px 6px; border-radius:4px;">
    ...
    <section id="bo_v_info">
        ...
        <?php
$is_all_day = isset($view['wr_5']) && $view['wr_5'] == '1'; // 종일 여부 체크

if ($view['wr_1'] == $view['wr_2']) {
    echo '<span class="date-txt">';
    if ($is_all_day) {
        echo '<span class="all-day-label">종일</span> ';
    }
    else {
        echo '<span class="all-day-time-label">부분</span> ';
    }
    echo wz_get_hangul_date1($view['wr_1']).'</span>';
}
else {
    echo '<span class="date-txt">';
    if ($is_all_day) {
    ...
    echo wz_get_hangul_date1($view['wr_1']).'</span> - <span class="date-txt">'.wz_get_hangul_date1($view['wr_2']).'</span>';
}
?>
        </div>
...
?>
    ...
    <div id="bo_v_top">
        ...
        <ul class="bo_v_left">
            <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" class="btn_b01 btn"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> 일정 수정</a></li><?php } ?>
            <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" class="btn_b01 btn" onclick="del(this.href); return false;"><i class="fa fa-trash-o" aria-hidden="true"></i> 일정 삭제</a></li><?php } ?>
            <?php if ($copy_href) { ?><li><a href="<?php echo $copy_href ?>" class="btn_admin btn" onclick="board_move(this.href); return false;"><i class="fa fa-files-o" aria-hidden="true"></i> 일정 복사</a></li><?php } ?>
            <?php if ($move_href) { ?><li><a href="<?php echo $move_href ?>" class="btn_admin btn" onclick="board_move(this.href); return false;"><i class="fa fa-arrows" aria-hidden="true"></i> 카데토리 이동</a></li><?php } ?>
        ...
        <ul class="bo_v_com">
           <li><a href="<?php echo $list_href ?>" class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i> 목록</a></li>
            ...
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02 btn"><i class="fa fa-pencil" aria-hidden="true"></i> 새 일정</a></li><?php } ?>
        </ul>
