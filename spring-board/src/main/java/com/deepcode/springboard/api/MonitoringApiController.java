...
public class MonitoringApiController {
    ...
    public Map<String, Object> getMonitoringChartData(HttpSession session) {
        log.info("API: Getting monitoring chart data");

        // 로그인 체크
        LoginUser user = (LoginUser) session.getAttribute(SessionConst.LOGIN_MEMBER);
        if (user == null) {
            log.warn("Unauthorized access to monitoring chart data");
        ...
        try {
            ...
            for (Map.Entry<Integer, List<MonitoringHistory>> entry : historyByService.entrySet()) {
                ...
                // CPU 사용량 데이터
                List<Double> cpuUsages = recentHistories.stream()
                        .map(h -> h.getMhCpuUsage() != null ? h.getMhCpuUsage() : 0.0)
                        .collect(Collectors.toList());
                ...
                serviceData.put("cpuUsages", cpuUsages);
