...
public class AdminMemberController {
    ...
    private final com.deepcode.springboard.mail.MailService mailService;
    ...
    @GetMapping("/mail")
    public String mail(@RequestParam(required = false) String mbId,
                       @ModelAttribute SearchCriteria criteria,
                       Model model,
                       HttpSession session,
                       RedirectAttributes redirectAttributes) {
        ...
        model.addAttribute("subMenu", "200100");
        model.addAttribute("pageTitle", "회원메일발송");
        model.addAttribute("csrfToken", com.deepcode.springboard.security.CsrfTokenManager.ensureToken(session));

        return "admin/member/mail";
    }

    @PostMapping("/mail")
    public String sendMail(@RequestParam(required = false) String[] mbIds,
        ...
        // CSRF 토큰 검증
        String sessionToken = com.deepcode.springboard.security.CsrfTokenManager.ensureToken(session);
        if (!com.deepcode.springboard.security.CsrfTokenManager.matches(sessionToken, _csrf)) {
            ...
            return "redirect:/mgmt/member/mail";
        }

        if (mbIds == null || mbIds.length == 0) {
            redirectAttributes.addFlashAttribute("error", "메일을 받을 회원을 선택하세요.");
            return "redirect:/mgmt/member/mail";
        }
        ...
        for (String mbId : mbIds) {
            ...
            try {
                mailService.sendMemberMail(member, subject, content);
                successCount++;
            } catch (Exception e) {
                failCount++;
            }
