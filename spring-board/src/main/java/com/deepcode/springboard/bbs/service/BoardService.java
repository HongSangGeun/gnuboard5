...
import com.deepcode.springboard.bbs.domain.WriteInsert;
import com.deepcode.springboard.bbs.domain.WriteUpdate;
import com.deepcode.springboard.bbs.mapper.BoardFileMapper;
import com.deepcode.springboard.bbs.mapper.BoardMapper;
import com.deepcode.springboard.bbs.mapper.WriteMapper;
...
import com.deepcode.springboard.common.HtmlSanitizer;
...
public class BoardService {
    ...
    public BoardService(BoardMapper boardMapper,
            ...
            @org.springframework.beans.factory.annotation.Autowired(required = false) OcrService ocrService,
            @org.springframework.beans.factory.annotation.Autowired(required = false) PdfService pdfService,
    ...
    public PagedResult<Write> listPosts(String boTable, int page, int size, String sfl, String stx, String sop,
        ...
        List<Write> posts = writeMapper.listPosts(table, offset, safeSize, safeSfl, safeStx, safeSop, sca, sdate, edate,
                sst, sod);
    ...
    public List<Write> listPostsByOffset(String boTable, int offset, int size, String sfl, String stx, String sop,
        ...
        String safeSfl = normalizeSearchField(sfl);
        String safeSop = normalizeSearchOperator(sop);
        return writeMapper.listPosts(table, safeOffset, safeSize, safeSfl, safeStx, safeSop, sca, sdate, edate, sst,
                sod);
    }
    ...
    public long createPost(String boTable, String subject, String content, String name, String password,
            ...
            String wr6, LoginUser loginUser, List<MultipartFile> files) {
        ...
        insert.setWr6(defaultString(wr6));
        ...
        if (wrId > 0) {
            processEditorImages(boTable, wrId, safeContent);
        }
        return wrId;
    }
    ...
    public void updatePost(String boTable, long wrId, String subject, String content, String name, String password,
            String email, String homepage, String ip, String caName, String wr1, String wr2, String wr4, String wr5,
            String wr6, LoginUser loginUser,
        ...
        // 삭제된 에디터 이미지 정리
        cleanupRemovedEditorImages(existing.getWrContent(), safeContent);
    ...
    public void deletePost(String boTable, long wrId, String password, LoginUser loginUser) {
        ...
        if (existing.getWrIsComment() == 1) {
            deleteEditorImages(existing.getWrContent());
    ...
    public List<BoardFile> listFiles(String boTable, long wrId) {
        ...
        List<BoardFile> allFiles = boardFileMapper.findByPost(safe, wrId);
        // 에디터 이미지는 첨부파일 목록에서 제외 (본문에 이미 표시됨)
        return allFiles.stream()
            .filter(f -> !"editor".equals(f.getBfStorage()))
            .toList();
    }
    ...
    private void ensureGroupAccess(Board board, LoginUser loginUser) {
        ...
        if (group == null || group.getGrUseAccess() != 1) {
            // Even if access check is unused, device check might still apply?
            // Usually frame/layout/skin handles device, but strict blocking is here.
            // Requirement says "Board Group Management ... service availability".
            // So we should check it regardless of grUseAccess (which is for member
            // level/group membership).
            // But legacy G5 usually checks device always. To be safe, let's check it if
            // group exists.
            if (group != null) {
                checkGroupDeviceAccess(group);
            }
            return;
        }
    ...
    private void checkGroupDeviceAccess(BoardGroup group) {
        String device = group.getGrDevice();
        if (device == null || device.isEmpty() || "both".equals(device)) {
            return;
        }

        boolean isMobile = isMobileRequest();

        if ("pc".equals(device) && isMobile) {
            throw new BoardPermissionDeniedException("PC 전용 게시판입니다.");
        }

        if ("mobile".equals(device) && !isMobile) {
            throw new BoardPermissionDeniedException("모바일 전용 게시판입니다.");
        }
    }
    ...
    private boolean isMobileRequest() {
        try {
            ...
            Object flag = attrs.getAttribute("isMobileView",
                    org.springframework.web.context.request.RequestAttributes.SCOPE_REQUEST);
            return Boolean.TRUE.equals(flag);
        } catch (NoClassDefFoundError | Exception e) {
            // In case of non-web context or missing dependencies (e.g. strict unit tests)
            return false;
        }
    }
    ...
    private void ensureWriteTableSchema(String table) {
        ...
        // MySQL 8.0 호환성: sql_mode 설정
        jdbcTemplate.execute("SET SESSION sql_mode='NO_ENGINE_SUBSTITUTION'");
    ...
    private void saveFiles(String boTable, long wrId, List<MultipartFile> files) {
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                // 텍스트 추출 (이미지 또는 PDF 파일)
                java.nio.file.Path filePath = fileStorageService.resolveBoardDir(safeBoTable)
                        .resolve(stored.storedName());
                String extractedText = null;

                try {
                    // PDF 파일 처리
                    if (isPdfFile(stored.originalName()) && pdfService != null) {
                        extractedText = pdfService.extractText(filePath.toFile());
                    }
                    // 이미지 파일 OCR 처리
                    else if (isImageFile(stored.originalName()) && ocrService != null && ocrService.isOcrAvailable()) {
                        extractedText = ocrService.extractText(filePath.toFile());
                    }

                    if (extractedText != null && !extractedText.trim().isEmpty()) {
                        bf.setBfOcrText(extractedText);
                    }
                } catch (Exception e) {
                    // 텍스트 추출 실패 시 로그만 남기고 계속 진행
                    System.err.println("Text extraction failed for file: " + stored.originalName() + " - " + e.getMessage());
                }
     ...
     * 조회수 증가
     */
    public void incrementHit(String boTable, long wrId) {
        String table = resolveWriteTable(boTable);
        ensureWriteTableSchema(table);
        writeMapper.incrementHit(table, wrId);
    }

    /**
     * 에디터 이미지에서 OCR 텍스트 추출 및 BoardFile 등록
     */
    private void processEditorImages(String boTable, long wrId, String content) {
        if (content == null || content.isBlank()) {
            return;
        }

        // 이미지 URL 패턴: /datahub/board/uploads/editor/(ckeditor4|cheditor5)/filename.ext
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(
            "/uploads/editor/(ckeditor4|cheditor5)/([^\"'\\s<>]+)");
        java.util.regex.Matcher matcher = pattern.matcher(content);

        Integer maxNo = boardFileMapper.findMaxNo(boTable, wrId);
        int bfNo = (maxNo != null) ? maxNo : 0;

        while (matcher.find()) {
            String editor = matcher.group(1);
            String filename = matcher.group(2);

            // OCR 텍스트 파일 확인
            java.nio.file.Path editorDir = java.nio.file.Paths.get(g5Properties.getDataPath(), "editor", editor);
            java.nio.file.Path ocrFile = editorDir.resolve(filename + ".ocr.txt");

            if (java.nio.file.Files.exists(ocrFile)) {
                try {
                    String ocrText = java.nio.file.Files.readString(ocrFile);
                    if (ocrText != null && !ocrText.trim().isEmpty()) {
                        // BoardFile 엔트리 생성
                        bfNo++;
                        BoardFile boardFile = new BoardFile();
                        boardFile.setBoTable(boTable);
                        boardFile.setWrId(wrId);
                        boardFile.setBfNo(bfNo);
                        boardFile.setBfSource(filename);
                        boardFile.setBfFile(filename);
                        boardFile.setBfDownload(0);
                        boardFile.setBfContent("");
                        boardFile.setBfFileurl("");
                        boardFile.setBfThumburl("");
                        boardFile.setBfStorage("editor");
                        boardFile.setBfFilesize(0);
                        boardFile.setBfWidth(0);
                        boardFile.setBfHeight(0);
                        boardFile.setBfType(1); // 1 = 이미지
                        boardFile.setBfDatetime(nowString());
                        boardFile.setBfOcrText(ocrText);

                        boardFileMapper.insertFile(boardFile);
                    }
                } catch (Exception e) {
                    // OCR 파일 처리 실패해도 계속 진행
                }
            }
        }
    }
    ...
    /**
     * 에디터 이미지 및 OCR 텍스트 파일 삭제
     */
    private void deleteEditorImages(String content) {
        if (content == null || content.isBlank()) {
            return;
        }
        ...
        while (matcher.find()) {
            ...
            try {
                ...
                // 이미지 파일 삭제
                java.nio.file.Path imageFile = editorDir.resolve(filename);
                if (java.nio.file.Files.exists(imageFile)) {
                ...
                java.nio.file.Path ocrFile = editorDir.resolve(filename + ".ocr.txt");
                if (java.nio.file.Files.exists(ocrFile)) {
                    java.nio.file.Files.delete(ocrFile);
                }
            } catch (Exception e) {
                // 파일 삭제 실패해도 계속 진행
            }
    ...
    /**
     * 게시글 수정 시 제거된 에디터 이미지 정리
     */
    private void cleanupRemovedEditorImages(String oldContent, String newContent) {
        ...
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(
            "/uploads/editor/(ckeditor4|cheditor5)/([^\"'\\s<>]+)");
        java.util.regex.Matcher matcher = pattern.matcher(oldContent);
        while (matcher.find()) {
            oldImages.add(matcher.group(1) + "/" + matcher.group(2));
        }
        ...
        if (newContent != null && !newContent.isBlank()) {
            ...
            while (matcher.find()) {
                newImages.add(matcher.group(1) + "/" + matcher.group(2));
            }
        }
        ...
        for (String oldImage : oldImages) {
            if (!newImages.contains(oldImage)) {
                String[] parts = oldImage.split("/");
                if (parts.length == 2) {
                    String editor = parts[0];
