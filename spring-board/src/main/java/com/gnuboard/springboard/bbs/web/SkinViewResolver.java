...
import com.gnuboard.springboard.config.MobileViewInterceptor;
...
public class SkinViewResolver {
    ...
    public String resolve(Board board, String view) {
        ...
        if (!skin.isBlank()) {
            // Priority 1: templates/skin/board/{skin}/{view}.html (Legacy/Custom support)
            String candidate = "skin/board/" + skin + "/" + view;
            Resource resource = resourceLoader.getResource("classpath:/templates/" + candidate + ".html");
            if (resource.exists()) {
                return candidate;
            }
            // Priority 2: templates/bbs/{skin}/{view}.html
            candidate = "bbs/" + skin + "/" + view;
            resource = resourceLoader.getResource("classpath:/templates/" + candidate + ".html");
            if (resource.exists()) {
                return candidate;
            }
        }
