...
public class WriteTableSqlProvider {
    ...
    public String listPosts(Map<String, Object> params) {
        ...
        if (hasSearchFilter) {
            sql.append("select p.* from ").append(table).append(" p ")
               ...
               .append("select distinct s.wr_parent from ").append(table).append(" s where 1=1");
            buildSearchCondition(sql, params, "s", sfl, stx, sop, sca, sdate, edate);
            sql.append(")");
        } else {
    ...
    public String updateWrite(Map<String, Object> params) {
        ...
        if (params.get("w") != null && params.get("w") instanceof com.gnuboard.springboard.bbs.domain.WriteUpdate) {
            com.gnuboard.springboard.bbs.domain.WriteUpdate w = (com.gnuboard.springboard.bbs.domain.WriteUpdate) params.get("w");
            if (w.getCaName() != null) {
