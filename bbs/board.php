...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            ...
            alert($msg, G5_BBS_URL.'/login?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_url($bo_table, $wr_id, $qstr)));
        }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_url($bo_table, $wr_id, $qstr)));
    }
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            ...
            alert('목록을 볼 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
    }
